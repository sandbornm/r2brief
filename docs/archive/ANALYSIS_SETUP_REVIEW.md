# R2B Analysis Setup Review - 2026-01-22

> Historical review. Current setup and interfaces: [README.md](README.md).

## Executive Summary

This document provides a comprehensive review of the r2b binary analysis platform's current setup, focusing on:
...
The Ghidra headless integration uses a custom Java script (`R2BHeadless.java`) that runs in batch mode:

**Location**: `ghidra/extensions/r2b/scripts/R2BHeadless.java`
...
**Python Integration**:
```python
# src/r2b/adapters/ghidra.py
def _headless_deep_scan(self, binary: Path, ...) -> dict[str, Any]:
    # Runs analyzeHeadless with R2BHeadless.java script
    # Parses JSON output with functions, strings, decompiled code
    # Returns structured analysis results
```

**Key Features**:
- Uses environment variable `R2B_OUTPUT` to specify JSON output path
...
**Location**: `src/r2b/adapters/ghidra_bridge_client.py`
...
**Python API**:
```python
from r2b.adapters.ghidra_bridge_client import GhidraBridgeClient
...
**Location**: `src/r2b/web/app.py:2102-2156`
...
**Python Integration**:
```python
# src/r2b/adapters/gef.py
from r2b.adapters.gef import GEFAdapter

gef = GEFAdapter(image="r2b-gef", timeout=60, max_instructions=10000)
...
- "Ask model" integration for code explanation
...
This context is passed to the LLM when users click "Ask model" or press `?` in the CFG viewer, enabling contextual code explanation based on the current view.
...
- Toggle via `localStorage.r2b_debug`
...
**Location**: `src/r2b/web/app.py` (activity_events table)
...
**Location**: `src/r2b/web/app.py:1671-1703` (`_get_activity_context`)
...
- Track "Ask model" interactions with context
...
uv run r2b analyze samples/bin/arm64/hello --ghidra
...
localStorage.setItem('r2b_debug', 'true');  // Enable debug mode
...
The r2b platform provides a robust foundation for binary analysis with:
