...
**SUPERSEDED.** Do not implement. External harnesses (omp / Claude Code /
Grok) shell out to `r2b`; do not absorb a planner into this repo.

---

# Unified Tool Execution & UI Simplification (historical)
...
**Tech Stack:** Python 3.11+, Pydantic v2 (strict validation), Flask, React 18, TypeScript, uv, Ruff, ty
...
**Files:**
- Create: `src/r2b/tools/__init__.py`
- Create: `src/r2b/tools/models.py`
...
Expected: ModuleNotFoundError: No module named 'r2b.tools'

**Step 3: Write minimal implementation**

```python
# src/r2b/tools/__init__.py
"""Tool execution system for r2b."""
...
```bash
uv run ruff check src/r2b/tools/ && uv run ty check src/r2b/tools/
```

**Step 6: Commit**

```bash
git add src/r2b/tools/ tests/unit/test_tool_models.py
...
**Files:**
- Create: `src/r2b/tools/validator.py`
...
from r2b.tools.models import ScriptLanguage, ToolName
...
Update `src/r2b/tools/__init__.py`:
...
```bash
git add src/r2b/tools/ tests/unit/test_script_validator.py
...
**Files:**
- Create: `src/r2b/tools/executor.py`
...
from r2b.tools.executor import ToolExecutor, GhidraExecutor
from r2b.tools.models import (
    ExecutionStatus,
    ScriptLanguage,
    ToolName,
...
```bash
git add src/r2b/tools/ tests/unit/test_tool_executor.py
...
**Files:**
- Modify: `src/r2b/adapters/ghidra_bridge_client.py`
...
Add to `src/r2b/adapters/ghidra_bridge_client.py` (after existing methods):
...
_r2b_output = io.StringIO()
_r2b_old_stdout = sys.stdout
sys.stdout = _r2b_output

try:
{self._indent_script(script)}
finally:
    sys.stdout = _r2b_old_stdout

_r2b_result = _r2b_output.getvalue()
'''
        ...
        try:
            ...
            output = namespace.get("_r2b_result", "")
...
```bash
uv run ruff check src/r2b/adapters/ghidra_bridge_client.py && uv run ty check src/r2b/adapters/ghidra_bridge_client.py
```

**Step 6: Commit**

```bash
git add src/r2b/adapters/ghidra_bridge_client.py tests/unit/test_ghidra_bridge.py
...
7. **Tooling** (Task 11): Enhanced Ruff/ty configuration
