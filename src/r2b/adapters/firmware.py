...
_BUILD_PATH_PATTERN = re.compile(
    r"(?:^|/)(?:cellar|toolchain|sysroot)(?:/|$)|"
    r"(?:^|/)[^/\x00]+\.o(?:$|[^a-z0-9])"
)
...
_STRING_RULES: tuple[_StringRule, ...] = (
    _StringRule(
        ...
        re.compile(
            r"(?:^|[^a-z0-9])(?:password|passwd|credential|secret|token)(?:$|[^a-z0-9])"
            r"|(?:^|[^a-z0-9])pwd=|(?:^|[^a-z0-9])default[-_ ]?login(?:$|[^a-z0-9])"
        ),
        0.82,
    ),
    _StringRule(
    ...
    _StringRule(
        ...
        re.compile(
            r"https?://|ftp://|"
            r"(?:^|[^a-z0-9])(?:tftp|telnet|dropbear|ssh|upnp|dnsmasq|socket|connect)(?:$|[^a-z0-9])"
        ),
        0.78,
    ),
    _StringRule(
    ...
    _StringRule(
        ...
        re.compile(
            r"(?:^|[^a-z0-9])(?:telnetd|sshd|httpd|uhttpd|lighttpd|boa|busybox|smbd|ftpd|dropbear)(?:$|[^a-z0-9])"
        ),
        0.8,
    ),
    _StringRule(
        "crypto",
        "Cryptography-related string",
        re.compile(
            r"(?:^|[^a-z0-9])(?:openssl|mbedtls|wolfssl|ssl|tls|x509|certificate|cert|aes|rsa|md5)(?:$|[^a-z0-9])"
            r"|(?:^|[^a-z0-9])sha(?:1|224|256|384|512)(?:$|[^a-z0-9])"
        ),
        0.72,
    ),
    _StringRule(
        "build_path",
        "Build or toolchain path",
        _BUILD_PATH_PATTERN,
        0.68,
    ),
    _StringRule(
        "filesystem",
        "Filesystem path or storage term",
        re.compile(
            r"/etc/|/bin/|/sbin/|/usr/|/var/|/tmp/|init\.d|rc\.d|"
            r"(?:^|[^a-z0-9])(?:mtd|nvram|uci)(?:$|[^a-z0-9])"
        ),
        0.76,
    ),
    _StringRule(
        "update",
        "Update or boot-related string",
        re.compile(
            r"(?:^|[^a-z0-9])(?:firmware|upgrade|update|flash|bootloader|u-boot|kernel|squashfs|ubi|mtdblock)(?:$|[^a-z0-9])"
        ),
        0.74,
    ),
    _StringRule(
        "dangerous_api",
        "Command or memory API string",
        re.compile(
            r"(?:^|[^a-z0-9])(?:system|exec|popen|strcpy|sprintf|memcpy|wget|curl|chmod|chown)(?:$|[^a-z0-9])"
        ),
        0.76,
    ),
)
...
class FirmwareAdapter:
    ...
    def quick_scan(self, binary: Path, **kwargs: Any) -> dict[str, Any]:
        ...
        if is_executable:
            ...
            # program; the same principle applies to PE and Mach-O subjects.
            artifacts = [
                artifact
                for artifact in artifacts
                if artifact.get("kind") not in {"jffs2_marker", "firmware_marker"}
                and not (
                    int(artifact.get("offset") or 0) == 0
                    and artifact.get("kind") == "elf_binary"
                )
            ]
        recommended_targets = []
    ...
    def _scan_string_signals(self, data: bytes) -> dict[str, Any]:
        ...
        for offset, value in self._iter_ascii_strings(data):
            ...
            for rule in _STRING_RULES:
                if rule.category == "filesystem" and _BUILD_PATH_PATTERN.search(lower):
                    continue
    ...
    def _output_dir(self, binary: Path) -> Path:
        ...
        return binary.parent / f"{binary.stem}.r2b-firmware"
    ...
    def _analysis_routing(self, artifact: dict[str, Any]) -> dict[str, Any]:
        ...
        if kind == "elf_binary" or signature == "elf":
            return {
                "analysis_role": "code",
                "fanout_tools": ["radare2", "ghidra", "angr"],
            }
    ...
    def _classify_top_level(self, data: bytes, artifacts: list[dict[str, Any]]) -> str:
        ...
        if data.startswith((b"\xfe\xed\xfa\xce", b"\xce\xfa\xed\xfe", b"\xfe\xed\xfa\xcf", b"\xcf\xfa\xed\xfe")):
            return "macho"
        if data.startswith((b"\xca\xfe\xba\xbe", b"\xbe\xba\xfe\xca", b"\xca\xfe\xba\xbf", b"\xbf\xba\xfe\xca")):
            return "fat_macho"
        if data.startswith(b"\xd0\r\xfe\xed"):
            return "fit_or_dtb"
        if data.startswith(b"\x27\x05\x19\x56"):
    ...
    def _classify_container(self, top_level_format: str, artifacts: list[dict[str, Any]]) -> str:
        ...
        if top_level_format in {"elf", "pe", "macho", "fat_macho"}:
            return "executable"
    ...
    def _notes(
        ...
        if isinstance(signal_counts, dict) and signal_counts:
            ...
            notes.append(f"Built-in string triage matched these categories: {categories}.")
