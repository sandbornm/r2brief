...
BRIEFING_SCHEMA_VERSION = "r2b.briefing.v1"
HANDOFF_SCHEMA_VERSION = "r2b.handoff.v1"
...
_CODE_SUBJECT_CLASSES = frozenset({"linux_elf", "baremetal_elf", "windows_pe", "macho"})
...
def build_briefing(
    ...
    *,
    ...
    artifact_dag = _dict(quick.get("artifact_dag"))
...
def _link_region_evidence(briefing: dict[str, Any], provenance: dict[str, Any]) -> None:
    """Attach compact provenance pointers to each region's evidence capsule."""
    actions = [item for item in _list(provenance.get("actions")) if isinstance(item, dict)]
    aliases = {"r2": "radare2"}
    for region in _list(briefing.get("regions")):
        if not isinstance(region, dict):
            continue
        snippet = _dict(region.get("snippet"))
        source = aliases.get(str(snippet.get("source") or ""), str(snippet.get("source") or ""))
        refs = [
            {
                "action": item.get("sequence"),
                "result_ref": item.get("result_ref"),
                "output_sha256": item.get("output_sha256"),
            }
            for item in actions
            if source and item.get("adapter") == source and item.get("result_ref")
        ]
        if refs:
...
def build_handoff(
    ...
    *,
    ...
    """Compact, deterministic view for a planner. No model, no ask templates."""
    binary = str(briefing.get("binary") or "")
    quoted = shlex.quote(binary) if binary else "BIN"
    subject = _dict(briefing.get("subject"))
    subject_class = str(subject.get("subject_class") or "")
    is_container = subject_class in _CONTAINER_SUBJECT_CLASSES
    requires_scope = subject.get("triage_scope") == "broad"
    ...
    if requires_scope:
        ...
        next_argv.extend(child_cmds[:4])
        if not child_cmds:
            next_argv.append(f"r2b brief {quoted} --extract --json")
    else:
    ...
    return {
        ...
        "subject": {
            ...
            "triage_scope": subject.get("triage_scope"),
            "scope_warnings": list(subject.get("scope_warnings") or []),
        },
        "regions": regions_out,
...
def _child_brief_cmd(action: str) -> str | None:
    match = _BRIEF_CHILD_RE.search(action)
    if not match:
        return None
    path = match.group(1).strip("`'\"")
    if not path or path in _PLACEHOLDER_PATHS or path.startswith("<"):
        return None
    return f"r2b brief {shlex.quote(path)} --quick --json"
...
def render_briefing_markdown(
    ...
    *,
    ...
    verified = data.get("verified_imports") or []
    if verified:
        lines.append("Verified imports:")
    ...
    argv = list((_dict(data.get("handoff")).get("next_argv")) or [])
    if argv:
        lines.extend(["", "## Next argv"])
        for cmd in argv:
            lines.append(f"- `{cmd}`")
    return "\n".join(lines).strip() + "\n"
...
def _subject(
    ...
    subject_class = _classify_subject(firmware, normalized_info, profile)
    try:
        size_bytes = Path(binary).stat().st_size
    except OSError:
        size_bytes = 0
    broad_target = subject_class in _CODE_SUBJECT_CLASSES and (
        size_bytes >= 32 * 1024 * 1024 or len(imports) >= 1000 or function_count >= 5000
    )
    scope_warnings: list[str] = []
    if broad_target:
        scope_warnings.append(
            "Large or high-cardinality code target: generic import/string ranking has high base-rate noise."
        )
        scope_warnings.append(
            "Narrow by dependency, crash address, export, subsystem, or version diff before trusting rank order."
        )
    return {
        "name": name,
        "binary": binary,
        "format": format_info.get("format") or firmware.get("top_level_format") or core.get("format") or profile.get("file_type"),
        "arch": _arch_label(normalized_info, profile),
        ...
        "risk_level": "unscored" if broad_target else profile.get("risk_level"),
        "risk_factors": list(profile.get("risk_factors") or [])[:6],
        "size_bytes": size_bytes,
        "triage_scope": "broad" if broad_target else "focused",
...
def _classify_subject(
    ...
    if is_elf:
        ...
        baremetal_os = os_name in {"none", "baremetal", "bare-metal", "unknown"}
        if baremetal_os:
            return "baremetal_elf"
        return "linux_elf"
    if top == "pe":
        return "windows_pe"
    if top in {"macho", "fat_macho"}:
        return "macho"
...
def _collect_regions(
    *,
    ...
    if subject_class not in _CODE_SUBJECT_CLASSES:
        regions.extend(_firmware_regions(firmware))
        regions.extend(_dag_regions(artifact_dag))
        ...
        regions.append(
            RegionAsk(
                ...
                snippet=RegionSnippet(
                    ...
                    text="\n".join(lines),
                    address=offset,
                    artifact_id=f"fw:{kind}:{offset or title}",
                    xref=str(artifact.get("name") or kind),
                ),
...
def _dag_regions(dag: dict[str, Any]) -> list[RegionAsk]:
    if not dag or not dag.get("nodes"):
        return []
    preferred = {"elf", "filesystem", "partition", "script"}
    hits: list[dict[str, Any]] = []
    for node in dag.get("nodes") or []:
        if not isinstance(node, dict):
            continue
        if node.get("properties", {}).get("root"):
            continue
    ...
    lines = []
    for node in hits:
        offset = node.get("parent_offset")
        tools = ",".join(str(t) for t in (node.get("source_tools") or [])[:3]) or "?"
        sha = str(node.get("sha256") or "")[:16]
        lines.append(f"{node.get('kind')} @ {offset} sha={sha or '-'} via {tools}  {node.get('label')}")
    first = hits[0]
    return [
        RegionAsk(
            id="dag:children",
            title="Extracted artifact map",
            why="Sandboxed extractors plus inventory, content-addressed; pick an ELF/fs child, not the wrapper.",
            score=94,
            tags=["artifact_dag", str(first.get("kind") or "file")],
            snippet=RegionSnippet(
                source="artifact_dag",
                kind="inventory",
                text="\n".join(lines),
                address=_hex(first.get("parent_offset")),
                artifact_id=str(first.get("id") or "dag:children"),
                xref=str(first.get("label") or first.get("kind") or ""),
            ),
            next_actions=[
                f"r2b brief <child ELF> --quick  # sha {str(first.get('sha256') or '')[:16] or 'from DAG'}",
                "Nukable store: artifacts_dir/extract plus records/",
            ],
        )
    ]
        ...
        regions.append(
            RegionAsk(
                ...
                snippet=RegionSnippet(
                    ...
                    artifact_id=f"signal:{category}",
                    xref=_clamp(str(first.get("value") or category), 40),
                ),
                next_actions=[
    ...
    for analysis in _list(children.get("analyses"))[:6]:
        ...
        regions.append(
            RegionAsk(
                ...
                next_actions=[
                    f"r2b brief {shlex.quote(target)} --quick --json",
    ...
    for issue in issues[:3]:
        ...
        regions.append(
            RegionAsk(
                ...
                score=72,
                tags=["issue"],
                snippet=RegionSnippet(source="r2b", kind="inventory", text=text),
    ...
    if snippet:
        lines.append(
            evidence_block(
                ...
                artifact=snippet.artifact_id or region.id,
                xref=snippet.xref or snippet.function,
            )
        )
    else:
        lines.append(evidence_block(tool="r2b", artifact=region.id))
...
def _overall_ask(subject: dict[str, Any], regions: list[RegionAsk], next_steps: list[str]) -> str:
    ...
    if subject.get("subject_class") in _CONTAINER_SUBJECT_CLASSES and subject.get("firmware_kind"):
...
def _overall_next_steps(
    ...
    if subject.get("triage_scope") == "broad":
        return [
            ...
            "Treat aggregate imports, strings, and generic region scores as inventory, not a vulnerability verdict.",
        ]
    ...
    elif subject_class in {"linux_elf", "baremetal_elf"}:
...
def _firmware_next_actions(kind: str, artifact: dict[str, Any]) -> list[str]:
    ...
    if kind == "elf_binary" and carved:
        return [
            f"r2b brief {shlex.quote(str(carved))} --quick --json",
            "Prefer this child over the container for Qwen asks.",
        ]
    if carved:
        return [
            f"r2b brief {shlex.quote(str(carved))} --quick --json",
            "Inspect with file(1) if this is not yet an ELF.",
        ]
...
def _summary_line(subject: dict[str, Any], regions: list[RegionAsk]) -> str:
    ...
    warning = (
        " Broad target: treat generic rank order as inventory until scope is narrowed."
        if subject.get("triage_scope") == "broad"
        else ""
    )
    return f"{'; '.join(parts)}. First region: {top}.{warning}"
...
def _basename(symbol: str) -> str:
    name = re.sub(r"^(sym\.imp\.|imp\.|sym\.)", "", symbol).split("@", 1)[0]
    name = re.sub(r"^_+imp_", "", name, flags=re.IGNORECASE)
    name = name.lstrip("_")
    if re.match(r"^[^_]+\.dll_", name, flags=re.IGNORECASE):
