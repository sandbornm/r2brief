...
GRAPH_SCHEMA_VERSION = "r2b.analysis_graph.v1"
...
class _GraphAccumulator:
    ...
    def add_tool(self, name: str, status: dict[str, Any] | bool | None) -> None:
        ...
        self.add_node(tool_id, kind="tool", label=name, source="r2b", properties=props)
        self.add_edge("observed_by", self.binary_node_id, tool_id, source_tool="r2b")
...
def build_analysis_graph(result: Any) -> dict[str, Any]:
    ...
    _add_artifact_dag(acc, quick.get("artifact_dag", {}) if isinstance(quick, dict) else {})
    ...
    for issue in getattr(result, "issues", []) or []:
        ...
        acc.add_node(issue_id, kind="issue", label=str(issue)[:120], source="r2b", properties={"message": issue})
        acc.add_edge("has_issue", acc.binary_node_id, issue_id, source_tool="r2b", confidence=0.8)
...
def _add_firmware_quick(acc: _GraphAccumulator, firmware: Any) -> None:
    if not isinstance(firmware, dict) or not firmware:
        return
    top_level_format = str(firmware.get("top_level_format") or "").lower()
    is_executable = top_level_format in {"elf", "pe", "macho", "fat_macho"}
    ...
    # Executable subjects already contribute native string/import nodes. Keep
    # firmware categorization in the adapter payload, but do not duplicate it
    # in the graph with a second, more interpretive label.
    firmware_signals = [] if is_executable else _as_list(
        (firmware.get("string_signals") or {}).get("top_signals")
    )[:100]
    for signal in firmware_signals:
    ...
    for item in _as_list(firmware.get("embedded_artifacts"))[:300]:
        ...
        if is_executable and int(offset or 0) == 0:
            continue
...
def _add_artifact_dag(acc: _GraphAccumulator, dag: Any) -> None:
    if not isinstance(dag, dict) or dag.get("schema_version") is None:
        return
    id_map: dict[str, str] = {}
    for node in _as_list(dag.get("nodes"))[:400]:
        if not isinstance(node, dict):
            continue
        # The artifact DAG carries compact code/string evidence for portable
        # records. The evidence map already receives those nodes from their
        # native adapters, so only merge byte/container topology here. This
        # avoids showing one import or string twice under different IDs.
        if str(node.get("kind") or "file") in {
            "function",
            "import",
            "string",
            "endpoint",
        ...
        if node.get("properties", {}).get("root"):
            id_map[raw_id] = acc.binary_node_id
            continue
        mapped = _node_id("dag", raw_id)
        id_map[raw_id] = mapped
        offset = node.get("parent_offset")
        acc.add_node(
            mapped,
            kind=str(node.get("kind") or "file"),
            label=str(node.get("label") or raw_id)[:100],
            source=",".join(str(t) for t in (node.get("source_tools") or ["artifact_dag"])[:4]) or "artifact_dag",
            address=_normalize_address(offset),
            properties=_pick(
                node,
                ["sha256", "parent_id", "parent_offset", "size", "path", "confidence", "properties"],
            ),
        )
    for edge in _as_list(dag.get("edges"))[:800]:
        ...
        if not src or not dst:
            continue
        acc.add_edge(
            str(edge.get("kind") or "contains"),
            src,
            dst,
            source_tool=str(edge.get("source_tool") or "artifact_dag"),
            properties={"parent_offset": edge.get("parent_offset")},
        )
