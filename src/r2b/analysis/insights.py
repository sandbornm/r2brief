...
INSIGHTS_SCHEMA_VERSION = "r2b.insights.v1"
MIN_SIBLINGS = 2
...
def _identity_patterns(dossiers: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Same SHA-256 under different names/paths is one binary, not two findings."""
    by_hash: dict[str, list[dict[str, Any]]] = defaultdict(list)
    for dossier in dossiers:
        digest = str(dossier.get("sha256") or dossier.get("record_id") or "")
        if digest:
            by_hash[digest].append(dossier)
    patterns = []
    for digest, members in by_hash.items():
        names = sorted({str(_label(item)) for item in members})
        paths = sorted({str(p) for item in members for p in (item.get("paths") or []) if p})
        if len(members) < MIN_SIBLINGS and len(paths) < MIN_SIBLINGS:
            continue
        if len(names) < 2 and len(paths) < 2:
            continue
        label = " / ".join(names[:4])
        patterns.append(
            _pattern(
                kind="identity",
                title=f"Identical ELF `{digest[:12]}` shipped as {label}",
                why="Same bytes, multiple firmware trees. Analyze once; do not double-count findings.",
                support=max(len(members), len(paths)),
                total=len(dossiers),
                evidence=[_sibling_ref(item) for item in members],
                next_action=f"r2b records show {digest[:16]}  # one record covers every path",
            )
        )
    return patterns
...
def _import_patterns(dossiers: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    # libc-ubiquitous: present in almost every ELF, never a finding.
    ubiquitous = {
        "close", "exit", "memset", "memcpy", "memmove", "memcmp",
        "malloc", "free", "calloc", "realloc",
        "fopen", "fclose", "fread", "fwrite", "fprintf", "printf", "sprintf",
        "strlen", "strcmp", "strncmp", "strcpy", "strncpy",
        "atoi", "atol", "strtol",
        "socket", "bind", "listen", "accept", "connect", "send", "recv",
        "setsockopt", "htons", "ntohs", "inet_addr",
        "pthread_mutex_lock", "pthread_mutex_unlock", "pthread_create",
        "time", "gettimeofday", "sleep", "usleep",
        "open", "read", "write", "lseek", "stat", "fstat",
        "signal", "sigaction", "abort", "perror",
    }
    for name, support in counts.most_common(24):
        if support < MIN_SIBLINGS:
            continue
        if name in ubiquitous and name not in hot:
            continue
        patterns.append(
            _pattern(
                kind="import",
    ...
    for family, support in counts.most_common(6):
        ...
        patterns.append(
            _pattern(
                ...
                next_action="r2b brief a carved child ELF (httpd/tdpServer), not the wrapper .bin.",
            )
        )
    return patterns
    ...
    for tag, support in counts.most_common(8):
        ...
        patterns.append(
            _pattern(
                ...
                why="Use this tag to pull the sibling set next time (`r2b insights --tag`).",
                support=support,
                total=total,
                evidence=owners[tag],
                next_action=f"r2b records list --tag {tag}",
            )
        )
    return patterns
