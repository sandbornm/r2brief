...
RECORD_SCHEMA_VERSION = "r2b.analysis_record.v1"
_MAX_STRINGS = 80
_MAX_FUNCTIONS = 400
_MAX_IMPORTS = 120
_MAX_CFG_FUNCS = 40
...
class AnalysisRecordStore:
    ...
    def persist(
        ...
        *,
        ...
        raw_path = binary or payload.get("binary") or ""
        path = Path(raw_path)
        ...
        dag = _dict(_dict(payload.get("quick_scan")).get("artifact_dag"))
        if dag:
            _write_json(directory / "graphs" / "artifact_dag.json", dag)
        ...
        artifact_manifest = {
            "schema_version": "r2b.record_artifacts.v1",
            "files": artifacts,
        }
        ...
        record = {
            ...
            "graphs": {
                ...
                "artifact_dag": "graphs/artifact_dag.json" if dag else None,
...
def _build_subject(
    ...
    *,
    ...
    format_info = _dict(quick.get("binary_format"))
    ...
    subject = {
        ...
        "arch": format_info.get("arch") or info.get("arch") or profile.get("architecture"),
        "bits": format_info.get("bits") or info.get("bits") or profile.get("bits"),
...
def _collect_cfgs(analysis: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for func in _list(r2.get("function_cfgs"))[:_MAX_CFG_FUNCS]:
        ...
        cfgs.append(
            {
                "schema_version": "r2b.function_cfg.v1",
                "source": "radare2",
                "name": func.get("name"),
                "offset": func.get("offset"),
                "size": func.get("size"),
...
def _collect_children(analysis: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    firmware = _dict(_dict(analysis.get("quick_scan")).get("firmware"))
    dag = _dict(_dict(analysis.get("quick_scan")).get("artifact_dag"))
    for node in _list(dag.get("nodes")):
        if not isinstance(node, dict):
            continue
        if node.get("kind") not in {"elf", "filesystem", "script"}:
            continue
        ...
        entry = {
            "path": node.get("path"),
            "kind": node.get("kind"),
            "role": "code" if node.get("kind") == "elf" else node.get("kind"),
            "offset": node.get("parent_offset"),
            "sha256": node.get("sha256"),
            "record_id": node.get("sha256"),
            "source_tools": node.get("source_tools") or [],
        }
...
def _update_index(records_root: Path, summary: dict[str, Any]) -> None:
    ...
    index = _read_json(path) or {"schema_version": "r2b.record_index.v1", "records": []}
    rows = [row for row in index.get("records") or [] if isinstance(row, dict)]
    ...
    rows.sort(key=lambda row: str(row.get("updated_at") or ""), reverse=True)
    _write_json(path, {"schema_version": "r2b.record_index.v1", "updated_at": utcnow(), "records": rows[:500]})
...
def _write_json(path: Path, payload: Any) -> None:
    """Atomic write: a crash mid-write must not tear the durable record."""
