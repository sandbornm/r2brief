...
DEFAULT_GHIDRA_INSTALL_ROOT = Path("~/.local/share/r2b/tools").expanduser()
...
def setup_ghidra(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="r2b-ghidra-") as tmp:
        tmp_path = Path(tmp)
...
def download_archive(url: str, destination: Path) -> None:
    ...
    request = urllib.request.Request(url, headers={"User-Agent": "r2b-ghidra-setup"})
    try:
