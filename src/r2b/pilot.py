"""Goal-driven pilot: planner LLM -> r2b execution -> grounded report.

The pilot drives the r2b CLI over a lab corpus root (a directory holding
``samples/`` and carved ``work/`` output). The explicitly configured provider
acts as planner, reporter, and
follow-up decider; every step runs as an ``r2b`` subprocess so a shifting
...
from .llm.credentials import requires_api_key, resolve_provider_base_url
from .llm.openai_client import OpenAIClient

VERBS = ("analyze", "brief", "records", "insights")
INTERESTING = (
...
PLANNER_SYSTEM = (
    ...
    "one of analyze|brief|records|insights, \"target\":string, "
    ...
    "target is a tag. Prefer breadth first, depth second."
)
...
FOLLOWUP_SYSTEM = (
    ...
    "records|insights, target string, ask string?, why string. "
...
class PilotLLM:
    """Planner/reporter chat over the configured provider endpoint."""
...
def corpus_context(root: Path) -> str:
    ...
    lines.append("R2B RECORDS: run `records list` step to enumerate; "
...
def r2b_invocation() -> list[str]:
    """Argv prefix that re-enters the r2b CLI (same environment)."""
    override = os.environ.get("R2B_PILOT_BIN")
    if override:
        return [override]
    found = shutil.which("r2b")
    if found:
        return [found]
    return [sys.executable, "-c", "from r2b.cli import run; run()"]
...
class PilotEngine:
    ...
    def preflight(self) -> tuple[bool, str]:
        """Probe the r2b CLI surface before spending LLM budget on it."""
        try:
            proc = subprocess.run(
                [*r2b_invocation(), "--help"],
                capture_output=True, text=True, timeout=120, cwd=str(self.root))
        except (OSError, subprocess.TimeoutExpired) as exc:
            return False, f"preflight r2b failed: {exc}"
        if proc.returncode != 0:
            return False, f"r2b --help rc={proc.returncode}: {proc.stderr[:200]}"
        missing = [v for v in VERBS if v not in (proc.stdout + proc.stderr)]
        if missing:
            return False, f"r2b CLI no longer advertises: {missing}"
        return True, "ok"
