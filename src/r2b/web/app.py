"""Flask web frontend for r2b with progress streaming."""
...
def create_app(config_path: Optional[Path] = None) -> Flask:
    ...
    checkout_dist = project_root / "web" / "frontend" / "dist"
    packaged_dist = Path(__file__).resolve().parents[1] / "web_dist"
    # A tracked placeholder keeps clean editable installs buildable even before
    # the frontend job runs. Prefer packaged assets only when a real bundle is
    # present; otherwise a contributor build in the checkout remains visible.
    dist_dir = packaged_dist if (packaged_dist / "index.html").is_file() else checkout_dist
    ...
    # This service can execute local analysis tools, so never opt every web
    # origin into credentialed API access.  Same-origin production builds do
    # not need CORS; the defaults below only cover the local Vite dev server.
    cors_value = os.getenv("R2B_CORS_ORIGINS") or os.getenv(
        "R2B_CORS_ORIGINS",
    ...
    def _start_compiler_status_refresh() -> None:
        ...
        thread = threading.Thread(target=_worker, name="r2b-compiler-sniffer", daemon=True)
        thread.start()
    ...
    def health() -> Any:
        ...
        return jsonify(_serialize({
            ...
            "llm": {
                ...
                "base_url": resolve_provider_base_url(state.config, key_env),
            },
            "features": {
    ...
    def _get_tools_status(state: AppState, *, live: bool = False) -> dict[str, Any]:
        ...
        tools["unblob"] = {
            ...
            "path": unblob_path,
            "install_hint": "Optional: apt install unblob",
            "description": "Sandboxed recursive extractor feeding the artifact DAG",
        }
        ...
        tools["angr"] = {
            ...
            "install_hint": "uv sync --extra symbolic",
        ...
        tools["libmagic"] = {
            ...
            "install_hint": "uv sync --extra r2; brew install libmagic (macOS)",
        ...
        tools["frida"] = {
            ...
            "install_hint": "uv sync --extra dynamic",
        ...
        tools["gef"] = {
            ...
            "install_hint": "docker build -t r2b-gef -f Dockerfile.gef .",
        ...
        tools["ghidra"] = {
            ...
            "install_hint": "Set GHIDRA_INSTALL_DIR; optional: uv sync --extra ghidra",
    ...
    def chat_bundle(session_id: str) -> Any:
        ...
        if output_format in {"md", "markdown"}:
            filename = f"{Path(session.binary_path).stem or session.session_id}-r2b-report.md"
            return Response(
        ...
        if output_format in {"zip", "archive"}:
            ...
            filename = f"{Path(session.binary_path).stem or session.session_id}-r2b-session.zip"
            return Response(
                archive.getvalue(),
    ...
    def post_chat_message(session_id: str) -> Any:
        ...
        if call_llm:
            ...
            from ..llm.citations import parse_cited_claims, proposed_annotations_from_claims

            cited = parse_cited_claims(assistant_response)
            if state.db:
                state.db.store_proposed_annotations(
                    session.session_id,
                    proposed_annotations_from_claims(cited),
                )
            metadata_attachment = [{
    ...
    def list_function_names(session_id: str) -> Any:
        ...
        with state.db.connect() as conn:
            rows = conn.execute(
                """
                SELECT id, address, original_name, display_name, reasoning, confidence, source,
                       status, artifact_id, tool, xref, created_at, updated_at
                FROM function_names
                WHERE session_id = ?
                ORDER BY address
                """,
        ...
        function_names = [
            {
                ...
                "source": row["source"],
                "status": row["status"] or ("proposed" if row["source"] == "llm" else "accepted"),
    ...
    def upsert_function_name(session_id: str) -> Any:
        ...
        status = body.get("status") or ("proposed" if source == "llm" else "accepted")
        ...
        with state.db.connect() as conn:
            conn.execute(
                ...
                INSERT INTO function_names (
                    id, session_id, address, original_name, display_name, reasoning, confidence,
                    source, status, artifact_id, tool, xref, created_at, updated_at
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT (session_id, address) DO UPDATE SET
                ...
                """,
                (
                    fn_id, session_id, address, original_name, display_name, reasoning, confidence,
                    source, status, artifact_id, tool, xref, now, now,
                ),
            )
    ...
    @app.get("/api/chats/<session_id>/proposed-annotations")
    def list_proposed_annotations(session_id: str) -> Any:
        if not state.db:
        ...
        with state.db.connect() as conn:
            rows = conn.execute(
                """
                SELECT id, kind, address, artifact_id, tool, xref, payload, status, source, created_at, updated_at
    ...
    @app.post("/api/chats/<session_id>/proposed-annotations")
    def upsert_proposed_annotation(session_id: str) -> Any:
        if not state.db:
            return jsonify({"error": "database not configured"}), 503
        session = chat_dao.get_session(session_id)
        if not session:
            return jsonify({"error": "session not found"}), 404
        body = request.get_json(silent=True) or {}
        kind = str(body.get("kind") or "claim")
        address = (body.get("address") or "").strip() or None
        status = str(body.get("status") or "proposed")
        payload = body.get("payload") if isinstance(body.get("payload"), dict) else {"text": body.get("text")}
        stored = state.db.store_proposed_annotations(
            session_id,
            [
                {
                    "id": body.get("id"),
        ...
        return jsonify({"stored": stored, "status": status, "kind": kind})
    ...
    def suggest_function_names() -> Any:
        ...
        prompt_parts = [
            ...
            "Names are PROPOSALS, not facts. Original names stay canonical until a human accepts.",
            "For each function, provide:",
            "1. A concise, descriptive name (snake_case, no prefix)",
            "2. A confidence level (high/medium/low)",
            "3. Brief reasoning (1 sentence) citing an address and a callee/xref from the listing",
            "4. cite: {tool, addr, xref} — tool is radare2 unless the listing says otherwise",
            "",
            "Respond in JSON format:",
            '{"suggestions": [{"address": "0x...", "name": "suggested_name", "confidence": "high|medium|low",'
            ' "reasoning": "why", "cite": {"tool": "radare2", "addr": "0x...", "xref": "puts"}}]}',
            "",
        ...
        try:
            llm_messages = [
                LLMChatMessage(
                    role="system",
                    content=(
                        "You are an expert reverse engineer. Suggest function names as proposals only. "
                        "Every suggestion needs cite.tool, cite.addr, and cite.xref from the listing. "
            ...
            if json_match:
                ...
                for s in suggestions:
                    ...
                    cite = s.get("cite") if isinstance(s.get("cite"), dict) else {}
                    s["cite"] = {
                        "tool": cite.get("tool") or "radare2",
                        "addr": cite.get("addr") or s.get("address"),
                        "xref": cite.get("xref"),
                        "artifact": cite.get("artifact") or cite.get("artifact_id"),
                    }
                    s["status"] = "proposed"
    ...
    def analyze() -> Any:
        ...
        enable_angr = bool(body.get("enable_angr", state.config.analysis.enable_angr))
        enable_ghidra = bool(body.get("enable_ghidra", state.config.analysis.enable_ghidra))
        enable_gef = bool(body.get("enable_gef", state.config.analysis.enable_gef))
        enable_frida = bool(body.get("enable_frida", state.config.analysis.enable_frida))
        require_elf = body.get("require_elf")
        if analysis_profile == "exhaustive":
            # Exhaustive broadens static/symbolic work. Native execution
            # remains an explicit request or a deliberate config choice.
            if "enable_angr" not in body:
                enable_angr = True
...
def _list_function_names(session: ChatSession, state: AppState) -> list[dict[str, Any]]:
    ...
    try:
        with state.db.connect() as conn:
            rows = conn.execute(
                """
                SELECT address, original_name, display_name, reasoning, confidence, source,
                       status, artifact_id, tool, xref, created_at, updated_at
                FROM function_names
                WHERE session_id = ?
                ORDER BY address
...
def _tool_speed_tier(name: str) -> str:
    ...
    if name == "radare2":
        return "medium"
    if name in {"angr", "ghidra", "gef", "frida"}:
        return "slow"
    if name == "ollama":
        return "interactive"
    return "unknown"
...
def _build_session_artifact_manifest(
    ...
    *,
    ...
    return {
        "schema_version": "r2b.session_artifact_manifest.v1",
...
def _render_analysis_bundle_markdown(bundle: dict[str, Any]) -> str:
    ...
    lines = [
        f"# r2b Analysis Report: {binary_name}",
        "",
    ...
    lines.extend(
        [
            ...
            "## Reproduce / Continue",
            "",
            "- Open the r2b session and use the Map tab for segmented findings.",
