...
def app_client(tmp_path: Path):
    ...
    with patch.dict('os.environ', {'R2B_WEB_DEBUG': 'false'}):
        # Patch the config used by build_state to keep database/artifacts isolated.
        with patch('r2b.state.load_config') as mock_config:
            from r2b.config import AppConfig, StorageSettings, AnalysisSettings, OutputSettings
...
class TestChatsEndpoint:
    ...
    def test_chat_analysis_without_attachment_returns_not_found(self, app_client, tmp_path):
        ...
        from r2b.storage.chat import ChatDAO
        from r2b.storage.db import Database
    ...
    def test_chat_briefing_returns_ranked_regions(self, app_client, tmp_path):
        ...
        assert response.status_code == 200
        data = response.get_json()
        assert data["briefing"]["schema_version"] == "r2b.briefing.v1"
    ...
    def test_chat_bundle_exports_json_and_markdown(self, app_client, minimal_elf):
        ...
        attachment = {
            ...
            "tool_availability": {"firmware": True},
            "tool_status": {"firmware": {"status": "completed", "functions_count": 0}},
            "evidence_coverage": {
                "columns": ["metadata"],
                "rows": ["angr"],
                "matrix": {"angr": {"metadata": "missing"}},
            },
            "analysis_graph": {
                "schema_version": "r2b.analysis_graph.v1",
                "binary": str(minimal_elf),
                ...
                "nodes": [
                    {"id": "issue:1", "kind": "issue", "label": "missing angr", "properties": {}},
        ...
        assert bundle["schema_version"] == "r2b.analysis_bundle.v1"
        assert bundle["schema_url"] == "schemas/analysis_bundle.schema.json"
        assert bundle["subject"]["sha256"] == "abc123"
        assert bundle["findings"]["evidence_gaps"] == ["angr: missing metadata"]
        assert bundle["tooling"]["tool_scorecard"]["firmware"]["quality"] == "good"
        assert bundle["manifest"]["schema_version"] == "r2b.session_artifact_manifest.v1"
        ...
        assert "# r2b Analysis Report" in markdown_response.get_data(as_text=True)
...
class TestAnalyzeEndpoint:
    ...
    def test_analyze_defaults_do_not_enable_native_execution(self, app_client):
        with patch('r2b.web.app.debug.analysis_start') as analysis_start:
            response = app_client.post(
                '/api/analyze',
                json={'binary': '/nonexistent/path/to/binary'},
                content_type='application/json',
        ...
        assert response.status_code == 404
        settings = analysis_start.call_args.args[1]
        assert settings['enable_gef'] is False
        assert settings['enable_frida'] is False

    def test_exhaustive_profile_keeps_native_execution_explicit(self, app_client):
        with patch('r2b.web.app.debug.analysis_start') as analysis_start:
            response = app_client.post(
                '/api/analyze',
                json={
                    'binary': '/nonexistent/path/to/binary',
                    'analysis_profile': 'exhaustive',
                },
                content_type='application/json',
            )
        ...
        assert settings['enable_ghidra'] is True
        assert settings['enable_gef'] is False
        assert settings['enable_frida'] is False
...
class TestChatMessagesEndpoint:
    ...
    def test_llm_error_returns_service_unavailable(self, app_client, minimal_elf):
        ...
        with patch('r2b.web.app.LLMBridge.chat', side_effect=LLMError("model missing")):
            response = app_client.post(
                f'/api/chats/{session_id}/messages',
