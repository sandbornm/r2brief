...
from r2b.environment.ghidra import detect_ghidra
from r2b.environment.ghidra_setup import (
    GhidraSetupError,
...
def test_detect_ghidra_uses_headless_on_path(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("PATH", str(headless.parent))
    monkeypatch.delenv("GHIDRA_INSTALL_DIR", raising=False)

    detection = detect_ghidra(AppConfig(), project_root=tmp_path)

    assert detection.install_dir == install_dir
    assert detection.headless_path == headless
    assert detection.is_ready is True
