...
from r2b.analysis.graph import GRAPH_SCHEMA_VERSION, build_analysis_graph
from r2b.analysis.investigation_graph import INVESTIGATION_SCHEMA_VERSION, build_investigation_graph
from r2b.analysis.orchestrator import AnalysisPlan, AnalysisResult
from r2b.storage.models import ChatMessage, ChatSession
    ...
    result = AnalysisResult(
        ...
        deep_scan={
            ...
            "angr": {
                ...
                tool_availability={"radare2": True, "angr": True},
    )
...
def test_analysis_graph_includes_artifact_dag_nodes(tmp_path: Path):
    result = AnalysisResult(
        binary=tmp_path / "wrapper.bin",
        plan=AnalysisPlan(),
        quick_scan={
            "artifact_dag": {
                "schema_version": "r2b.artifact_dag.v1",
                "sha256": "a" * 64,
                "nodes": [
                    {
                        "id": "n:container:root",
                ...
                "edges": [
                    {
                        "id": "e:contains:n:container:root:n:elf:child",
                        "kind": "contains",
                        "source": "n:container:root",
                        "target": "n:elf:child",
                        "source_tool": "binwalk3",
                    }
                ],
            }
        },
    ...
    assert contains["source"] == "binary:root"


def test_executable_graph_does_not_duplicate_firmware_string_or_root_signature(tmp_path: Path):
    result = AnalysisResult(
        binary=tmp_path / "sample.elf",
        plan=AnalysisPlan(),
        quick_scan={
            "firmware": {
                ...
                "string_signals": {
                    "top_signals": [
                        {
                            "category": "network",
                            "label": "Network-related string",
                            "value": "http://127.0.0.1/health",
                            "offset": 64,
                        }
                    ]
                },
                "embedded_artifacts": [
                    {"offset": 0, "kind": "elf_binary", "name": "ELF"}
                ],
            },
            "radare2": {
                "strings": [
                    {"string": "http://127.0.0.1/health", "vaddr": 0x400040}
                ]
            },
        },
    )
    ...
    matching = [
        ...
        if node["kind"] == "string" and node["label"] == "http://127.0.0.1/health"
    ]
    assert len(matching) == 1
    assert not any(node["kind"] == "embedded_artifact" for node in graph["nodes"])
