...
from r2b.llm.openai_client import (
    OpenAIClient,
    OpenAIError,
    _provider_base_url,
    _rate_limit_message,
...
def test_provider_base_url_keeps_ollama_default():
    config = AppConfig(llm=LLMSettings())
    assert _provider_base_url(config) == "http://127.0.0.1:11434"


def test_provider_base_url_uses_explicit_field():
    config = AppConfig(
        llm=LLMSettings(base_url="http://minimus-mac-mini:52415/v1/")
    )
    assert _provider_base_url(config) == "http://minimus-mac-mini:52415/v1"


def test_provider_base_url_from_provider_base_url():
    config = AppConfig(
        llm=LLMSettings(provider="openai", base_url="http://127.0.0.1:8000/v1")
    )
    assert _provider_base_url(config) == "http://127.0.0.1:8000/v1"
...
def test_resolve_prefers_named_api_key_env(monkeypatch):
    ...
    key, name = _resolve_provider_api_key(config)
    assert key == "zai-secret"
    assert name == "ZAI_API_KEY"


def test_z_ai_host_requires_a_key(monkeypatch):
    for name in ("ZAI_API_KEY", "XAI_API_KEY", "OPENAI_API_KEY", "GLM_API_KEY", "ZHIPUAI_API_KEY", "ZHIPU_API_KEY", "OPENROUTER_API_KEY"):
    ...
    assert _requires_api_key(config.llm.base_url) is True
    with pytest.raises(OpenAIError, match="ZAI_API_KEY"):
        OpenAIClient(config)


def test_x_ai_host_requires_a_key(monkeypatch):
    for name in ("ZAI_API_KEY", "XAI_API_KEY", "OPENAI_API_KEY", "GLM_API_KEY", "ZHIPUAI_API_KEY", "ZHIPU_API_KEY", "OPENROUTER_API_KEY"):
        monkeypatch.delenv(name, raising=False)
    config = AppConfig(
        llm=LLMSettings(
    ...
    assert _requires_api_key(config.llm.base_url) is True
    with pytest.raises(OpenAIError, match="XAI_API_KEY"):
        OpenAIClient(config)
...
def test_glm_provider_defaults_to_bigmodel_host(monkeypatch):
    ...
    assert _provider_base_url(config) == "https://open.bigmodel.cn/api/paas/v4"
    client = OpenAIClient(config)
...
def test_glm_provider_uses_z_ai_when_zai_key(monkeypatch):
    ...
    assert _provider_base_url(config) == "https://api.z.ai/api/paas/v4"
...
def test_rate_limit_message_distinguishes_z_ai_empty_credit():
    ...
    assert "429" in _rate_limit_message(Exception("Error code: 429 - rate limit exceeded")).lower()
    zai = _rate_limit_message(Exception("Error code: 429 - rpm"), base_url="https://api.z.ai/api/coding/paas/v4/")
    assert zai.startswith("Z.ai/GLM HTTP 429")
    assert "rpm" in zai
