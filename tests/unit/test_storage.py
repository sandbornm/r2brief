...
from r2b.storage import Database
from r2b.storage.dao import TrajectoryDAO
from r2b.storage.models import (
    ChatSession,
    ChatMessage,
    TrajectoryAction,
    AnalysisTrajectory,
...
class TestDatabase:
    ...
    def test_database_creates_tables(self, tmp_db_path):
        ...
        assert "proposed_annotations" in tables
        assert "function_names" in tables

    def test_store_proposed_annotations_is_stable(self, chat_dao):
        session = chat_dao.get_or_create_session("/tmp/test.bin")
        database = chat_dao._db
        row = {
            "kind": "name",
            "address": "0x401000",
            "artifact_id": "n:function:ab",
            "tool": "radare2",
            "xref": "strcmp",
            "payload": {"text": "proposed name http_auth"},
            "status": "proposed",
            "source": "llm",
        }
        first = database.store_proposed_annotations(session.session_id, [row])
        second = database.store_proposed_annotations(session.session_id, [row])
        assert first == 1
        assert second == 1
        with database.connect() as conn:
            count = conn.execute(
                "SELECT COUNT(*) FROM proposed_annotations WHERE session_id = ?",
                (session.session_id,),
            ).fetchone()[0]
        assert count == 1

    def test_llm_function_names_default_proposed(self, chat_dao):
        session = chat_dao.get_or_create_session("/tmp/test.bin")
        database = chat_dao._db
        with database.connect() as conn:
            conn.execute(
                ...
                INSERT INTO function_names (
                    id, session_id, address, original_name, display_name, source, created_at, updated_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """,
                ("fn-1", session.session_id, "0x1000", "sub_1000", "http_auth", "llm", "now", "now"),
            )
            conn.commit()
        ...
        with database.connect() as conn:
            status = conn.execute(
                "SELECT status FROM function_names WHERE id = ?",
                ("fn-1",),
            ).fetchone()[0]
