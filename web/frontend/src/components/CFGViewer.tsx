...
const CFGGraphBase: FC<CFGGraphProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        {onAskAboutCFG && (
          <Tooltip title="Ask model about this code (?)">
...
const CFGViewer: FC<CFGViewerProps> = ({
  ...
  const getDisplayName = useCallback((fn: FunctionCFG): { display: string; original: string; isRenamed: boolean; isProposed?: boolean; proposed?: string; source?: string; reasoning?: string } => {
    const fnName = functionNames.get(fn.offset);
    if (!fnName) {
      return { display: fn.name, original: fn.name, isRenamed: false };
    }
    const original = fnName.originalName || fn.name;
    if (fnName.status === 'rejected') {
      return { display: original, original, isRenamed: false };
    }
    const llmProposed = fnName.source === 'llm' && fnName.status !== 'accepted';
    if (llmProposed) {
      return {
        display: original,
        original,
    ...
    return {
      ...
      original,
      isRenamed: fnName.displayName !== original,
      source: fnName.source,
      reasoning: fnName.reasoning,
    };
  }, [functionNames]);
      ...
      for (const suggestion of suggestions) {
        ...
        await fetch(`/api/chats/${sessionId}/function-names`, {
          ...
          body: JSON.stringify({
            ...
            source: 'llm',
            status: 'proposed',
            tool: suggestion.cite?.tool,
            xref: suggestion.cite?.xref,
            artifactId: suggestion.cite?.artifact,
          }),
        });
  ...
  const handleSetNameStatus = useCallback(async (fn: FunctionCFG, status: 'accepted' | 'rejected') => {
    if (!sessionId) return;
    const existing = functionNames.get(fn.offset);
    const displayName = status === 'accepted'
      ? (existing?.displayName || fn.name)
      : (existing?.originalName || fn.name);
    ...
    try {
      await fetch(`/api/chats/${sessionId}/function-names`, {
        ...
        body: JSON.stringify({
          address: fn.offset,
          originalName: existing?.originalName || fn.name,
          displayName,
      ...
      setFunctionNames((prev) => {
        ...
        const current = next.get(fn.offset);
      ...
      // Ignore persistence errors; local name stays proposed until reload.
    }
  }, [sessionId, functionNames]);
                    ...
                    primary={
                      isEditing ? (
                        ...
                        <Tooltip
                          title={
                            nameInfo.isProposed
                              ? `proposed ${nameInfo.proposed} — ${nameInfo.reasoning || 'LLM suggestion, not accepted'}`
                              : nameInfo.isRenamed
                        ...
                        >
                          <Stack direction="row" alignItems="center" spacing={0.5}>
                            ...
                            {nameInfo.isProposed && nameInfo.proposed && (
                              <Typography
                                variant="caption"
                                sx={{ fontFamily: 'monospace', color: 'warning.main', fontStyle: 'italic' }}
                              >
                                ~{nameInfo.proposed.length > 16 ? nameInfo.proposed.slice(0, 16) + '…' : nameInfo.proposed}
                              </Typography>
                            )}
                            {nameInfo.isProposed && sessionId && (
                              <>
                                <Tooltip title={`accept proposed ${nameInfo.proposed}`}>
                                  <IconButton
                                    size="small"
                                    aria-label={`accept proposed name ${nameInfo.proposed}`}
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      handleSetNameStatus(fn, 'accepted');
                                    }}
                                ...
                                <Tooltip title="reject proposal; keep original">
                                  <IconButton
                                    size="small"
                                    aria-label={`reject proposed name ${nameInfo.proposed}`}
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      handleSetNameStatus(fn, 'rejected');
                                    }}
