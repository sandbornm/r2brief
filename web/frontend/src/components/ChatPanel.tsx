...
interface ChatPanelProps {
  ...
  onSend: (content: string, options: { callLLM: boolean }) => Promise<boolean>;
  sending?: boolean;
  error?: string | null;
  disassembly?: string;
...
const CopyButton: FC<{ text: string }> = memo(function CopyButton({ text }) {
  ...
  useEffect(() => () => clearTimeout(copyTimerRef.current ?? undefined), []);
...
const MessageBubble: FC<MessageBubbleProps> = memo(function MessageBubble({ message, disassembly, onNavigateToAddress, isLatest }) {
  ...
  const analysisAttachment = attachments.find(
    ...
    >
      ...
      >
        <Stack spacing={1.5}>
          ...
          {/* Provider badge + claim grounding */}
          {message.attachments
            .filter((a) => a.type === 'llm_response_meta')
            .map((a, i) => {
              const cited = a.cited_claims as
                | { claims?: unknown[]; uncited?: unknown[]; proposed?: unknown[]; grounded?: boolean }
                | undefined;
              const claimCount = Array.isArray(cited?.claims) ? cited.claims.length : 0;
              const uncitedCount = Array.isArray(cited?.uncited) ? cited.uncited.length : 0;
              const proposedCount = Array.isArray(cited?.proposed) ? cited.proposed.length : 0;
              return (
                <Stack key={i} direction="row" spacing={0.5} flexWrap="wrap" useFlexGap>
                  <Chip
                    size="small"
                  ...
                  {uncitedCount > 0 && (
                    <Chip
                      size="small"
                      color="warning"
                      label={`${uncitedCount} uncited — ungrounded`}
                      sx={{ height: 20, fontSize: '0.65rem' }}
                    />
                  )}
                  {proposedCount > 0 && (
                    <Chip
                      size="small"
                      label={`${proposedCount} proposed name/type`}
                      sx={{ height: 20, fontSize: '0.65rem' }}
                    />
                  )}
                </Stack>
...
export const ChatPanel: FC<ChatPanelProps> = memo(({
  ...
  useEffect(() => () => clearTimeout(scrollTimeoutRef.current ?? undefined), []);
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    const sent = await onSend(content, { callLLM });
    if (sent) setContent('');
  };
  // Filter visible messages (before the early return: hook count must not
  // depend on whether a session is active)
