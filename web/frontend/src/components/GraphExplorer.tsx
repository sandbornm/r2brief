...
const graphTitle = (mode: GraphMode) => mode === 'findings' ? 'Evidence Map' : 'Journey Map';
    ...
    >
      ...
      >
        ...
        >
          ...
          {signalCount > 0 && <Chip size="small" label={`${signalCount} pivots`} color="info" variant="outlined" sx={{ height: 22 }} />}
  ...
  return (
    ...
    >
      ...
      {segments.map((segment) => {
        ...
        const signalLabel = segment.importantCount === 1 ? 'pivot' : 'pivots';
        return (
          <Chip
            key={segment.key}
            size="small"
            ...
            >
              ...
              {important && (
                <rect
                  ...
                  rx="11"
                  fill="none"
                  stroke={alpha(theme.palette.info.main, isSegment ? 0.42 : 0.5)}
              ...
              {isSegment ? (
                  ...
                  {nodeDetailLevel !== 'compact' && (
                    <>
                      ...
                      {Number(node.properties.important_count ?? 0) > 0 && (
                        <>
                          <rect x="120" y="38" width="94" height="22" rx="11" fill={alpha(theme.palette.info.main, 0.16)} />
                          <text x="134" y="53" fill={theme.palette.info.main} fontSize="11" fontWeight="800">
                            {String(node.properties.important_count)} pivot
                          </text>
                        </>
                      )}
                    </>
                ...
                <>
                  ...
                  {nodeDetailLevel !== 'compact' && Number(node.properties.important_count ?? 0) > 0 && (
                    <text x={node.width - 16} y="44" fill={theme.palette.info.main} fontSize="10" fontWeight="800" textAnchor="end">
                      {String(node.properties.important_count)} pivot
                    </text>
                  )}
                  {nodeDetailLevel === 'full' && Array.isArray(node.properties.samples) && node.properties.samples.slice(0, 2).map((sample, index) => (
          ...
          {nodes.map((node) => {
            ...
            return (
              <rect
                ...
                stroke={important ? theme.palette.info.main : 'none'}
...
const GraphInspector: FC<GraphInspectorProps> = ({
  ...
  const prioritySegments = segments
    ...
    .slice(0, 6);
  const establishingTools = useMemo(() => {
    if (!node) return [];
    const tools = new Set<string>();
    addSourceValues(tools, node.source);
    addSourceValues(tools, node.properties.sources);
    addSourceValues(tools, node.properties.source_tools);
    for (const edge of graph?.edges ?? []) {
      if (edge.source !== node.id && edge.target !== node.id) continue;
      addSourceValues(tools, edge.source_tool);
    }
    return Array.from(tools).sort();
  }, [graph?.edges, node]);
  const evidenceStatus = node ? evidenceStatusForNode(node) : null;
      ...
      <Stack spacing={1.2} sx={{ p: 1.5, borderBottom: 1, borderColor: 'divider' }}>
        ...
        <Typography variant="caption" color="text.secondary">
          {segment && lens === 'segment'
            ...
            : mode === 'findings'
            ? 'Artifacts, static indicators, code relationships, and analysis issues.'
          ...
          <Stack spacing={1.5}>
            <Box>
              <Stack direction="row" spacing={0.75} flexWrap="wrap" gap={0.75} sx={{ mb: 1 }}>
                <Chip size="small" label={node.kind} sx={{ bgcolor: alpha(colorForKind(node.kind), 0.16), color: colorForKind(node.kind) }} />
                <Chip size="small" label="Evidence" variant="outlined" color="info" />
                {evidenceStatus && <Chip size="small" label={evidenceStatus} variant="outlined" />}
              </Stack>
            ...
            {(establishingTools.length > 0 || node.address) && (
              <Stack spacing={0.75}>
                <Typography variant="caption" color="text.secondary" fontWeight={700}>
                  Established by
                </Typography>
                {establishingTools.length > 0 && (
                  <Stack direction="row" flexWrap="wrap" gap={0.75}>
                    {establishingTools.map((tool) => (
                      <Chip key={tool} size="small" label={tool} variant="outlined" sx={{ height: 22 }} />
                    ))}
                  </Stack>
                )}
                {node.address && <InfoLine label="Address / offset" value={node.address} />}
              </Stack>
            )}
            ...
            {isAggregateNode(node) && (
              <Stack spacing={1}>
                <Stack direction="row" flexWrap="wrap" gap={0.75}>
                  ...
                  {Number(node.properties.important_count ?? 0) > 0 && (
                    <Chip size="small" label={`${String(node.properties.important_count)} pivots`} color="info" variant="outlined" sx={{ height: 22 }} />
                  )}
                  {Boolean(node.properties.grouped_kind) && (
              ...
              <>
                ...
                <Stack direction="row" flexWrap="wrap" gap={0.75}>
                  ...
                  {segment.importantCount > 0 && (
                    <Chip size="small" label={`${segment.importantCount} pivots`} color="info" variant="outlined" sx={{ height: 22 }} />
                  )}
                </Stack>
...
function aggregateBucket(node: ExplorerGraphNode): { key: string; label: string } | null {
  ...
  if (node.kind === 'string') {
    if (/https?:\/\/|ftp:\/\/|tftp|telnet|ssh|dropbear|dns|socket|host/.test(text)) {
      return { key: 'string:network', label: 'Network-related string matches' };
    }
    if (/password|passwd|token|secret|credential|auth|login|admin|root|private key/.test(text)) {
      return { key: 'string:auth', label: 'Authentication-related string matches' };
    }
    if (/ssl|tls|crypto|certificate|cert|rsa|aes|sha256|md5|key/.test(text)) {
      return { key: 'string:crypto', label: 'Cryptography-related string matches' };
    }
    if (/\/etc\/|\/bin\/|\/sbin\/|\/usr\/|\/tmp\/|busybox|init.d|rc.d|shell|sh /.test(text)) {
      return { key: 'string:filesystem-shell', label: 'Path and shell string matches' };
    }
    if (/firmware|upgrade|update|flash|boot|mtd|ubi|squashfs|kernel/.test(text)) {
      return { key: 'string:firmware-update', label: 'Update and boot string matches' };
    }
    return { key: 'string:other', label: 'Other string matches' };
  }

  if (node.kind === 'import') {
    if (/malloc|free|memcpy|memmove|strcpy|strncpy|sprintf|snprintf|gets|scanf/.test(text)) {
      return { key: 'import:memory', label: 'Memory and string API imports' };
    }
    if (/system|exec|popen|fork|spawn|shell|cmd/.test(text)) {
      return { key: 'import:process', label: 'Process API imports' };
    }
    if (/socket|connect|bind|listen|accept|send|recv|http|curl|dns/.test(text)) {
      return { key: 'import:network', label: 'Network API imports' };
    }
    if (/ssl|tls|crypto|aes|rsa|sha|md5|x509/.test(text)) {
      return { key: 'import:crypto', label: 'Cryptography API imports' };
    }
    if (/open|read|write|stat|chmod|unlink|rename|mount/.test(text)) {
      return { key: 'import:filesystem', label: 'Filesystem API imports' };
    }
  ...
  if (node.kind === 'function') {
    ...
    if (/parse|decode|decrypt|auth|login|update|flash|http|socket|memcpy|strcpy|system/i.test(node.label)) {
      return { key: 'function:keyword-match', label: 'Function-name matches' };
    }
  }
...
function addSourceValues(target: Set<string>, value: unknown): void {
  const values = Array.isArray(value) ? value : typeof value === 'string' ? value.split(',') : [];
  for (const item of values) {
    const source = String(item).trim();
    if (source && source !== 'merged') target.add(source);
  }
}

function evidenceStatusForNode(node: ExplorerGraphNode): string {
  const explicitStatus = typeof node.properties.status === 'string' ? node.properties.status.trim() : '';
  if (explicitStatus === 'proposed' || explicitStatus === 'accepted' || explicitStatus === 'rejected') {
    return explicitStatus;
  }
  const source = `${node.source ?? ''} ${node.actor ?? ''}`.toLowerCase();
  if (node.kind === 'issue') return 'analysis issue';
  if (source.includes('model') || node.actor === 'model') return 'proposed by model';
  if (node.actor === 'human' || node.kind === 'human_action') return 'analyst activity';
  if (/\bfrida\b|\bgef\b/.test(source) || node.properties.run_id) return 'runtime observation';
  if ([
    'embedded_artifact',
    'artifact_analysis',
    'artifact',
    ...
    return node.properties.sha256 ? 'byte-backed artifact' : 'detector match';
  }
  if (['section', 'function', 'basic_block', 'import', 'string', 'endpoint', 'config_key', 'decompilation', 'type'].includes(node.kind)) {
...
function isImportantNode(node: ExplorerGraphNode): boolean {
  ...
  if (node.kind === 'import') {
    return /malloc|free|memcpy|strcpy|sprintf|system|exec|popen|fork|spawn|socket|connect|bind|listen|accept|http|crypto|ssl|tls|dlopen|dlsym|open|read|write|ioctl|mmap|mount|rename|unlink/.test(text);
  }
  if (node.kind === 'string') {
    return /http|https|password|passwd|token|key|secret|auth|login|telnet|dropbear|busybox|admin|root|update|firmware|upgrade|flash|boot|\/etc\/|\/bin\/|\/sbin\/|uart|serial|i2c|spi/.test(text);
  }
  if (node.kind === 'function') {
    return /main|init|parse|decode|encode|compress|decompress|load|save|read|write|open|verify|sign|encrypt|decrypt|auth|login|update|flash|dispatch|handler|http|socket|ioctl|mmap|memcpy|strcpy|system|exec/.test(text);
  }
  return false;
}
...
function mapSignalScore(node: ExplorerGraphNode): number {
  ...
  if (node.kind === 'issue') score += 80;
  if (isImportantNode(node)) score += 60;
...
function mapSignalReason(node: ExplorerGraphNode): string {
  const groupCount = Number(node.properties.group_count ?? 0);
  const importantCount = Number(node.properties.important_count ?? 0);
  const groupedKind = String(node.properties.grouped_kind ?? '').replace(/_group$/, '');

  if (node.kind === 'issue') return 'analysis issue';
  if (importantCount > 0 && groupedKind === 'import') {
    return `${importantCount} import-name match${importantCount === 1 ? '' : 'es'} grouped`;
  }
  if (importantCount > 0 && ['string', 'endpoint', 'config_key'].includes(groupedKind)) {
    return `${importantCount} string match${importantCount === 1 ? '' : 'es'} grouped`;
  }
  if (importantCount > 0 && ['embedded_artifact', 'artifact_analysis', 'artifact'].includes(groupedKind)) {
    return `${importantCount} artifact signature match${importantCount === 1 ? '' : 'es'} grouped`;
  }
  if (importantCount > 0) return `${importantCount} evidence pivot${importantCount === 1 ? '' : 's'} grouped`;
  if (groupCount > 0) return `${groupCount} related item${groupCount === 1 ? '' : 's'} grouped`;
  if (node.kind === 'import') return `import-name match: ${node.label}`;
  if (node.kind === 'string' || node.kind === 'endpoint' || node.kind === 'config_key') {
    return `string match${node.address ? ` at ${node.address}` : ''}`;
  }
  if (['embedded_artifact', 'artifact_analysis', 'artifact', 'container', 'partition', 'filesystem', 'file', 'elf', 'script'].includes(node.kind)) {
    return `artifact signature match: ${node.label}`;
  }
  if (node.kind === 'function') return `function-name match: ${node.label}`;
  if (node.address) return `${node.kind} at ${node.address}`;
  return `${node.kind.replace(/_group$/, '').replace(/_/g, ' ')} evidence`;
}
...
function segmentMeta(mode: GraphMode, kind: string): { key: string; label: string; color: string; order: number } {
  ...
  if (['section', 'section_group', 'function', 'function_group', 'basic_block', 'basic_block_group', 'decompilation', 'type', 'elf'].includes(kind)) {
    return { key: 'code', label: 'Code', color: '#22c55e', order: 1 };
  }
  if (['import', 'import_group', 'string', 'string_group', 'endpoint', 'config_key'].includes(kind)) {
    return { key: 'indicators', label: 'Indicators', color: '#f59e0b', order: 2 };
  }
  if ([
    'embedded_artifact',
    'embedded_artifact_group',
  ...
  if (kind === 'issue') {
    return { key: 'issues', label: 'Issues', color: '#ef4444', order: 5 };
  }
  return { key: kind, label: titleCase(kind), color: colorForKind(kind), order: 20 };
}
