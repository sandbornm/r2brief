...
const ANNOTATIONS_STORAGE_KEY = 'r2b-annotations';
...
const deriveToolScorecard = (toolStatus: Record<string, ToolStatusSummary>): Record<string, ToolScorecardEntry> => {
  const speedFor = (name: string) => {
    ...
    if (['radare2'].includes(name)) return 'medium';
    if (['angr', 'ghidra', 'gef', 'frida', 'gdb'].includes(name)) return 'slow';
    return 'unknown';
  };
...
const ResultViewer: FC<ResultViewerProps> = memo(({ result, sessionId, onAskAboutCode, onAskAboutCFG }) => {
  ...
  const handleExportBundle = useCallback(async (format: 'json' | 'markdown' | 'zip') => {
    ...
    if (format === 'markdown') {
      ...
      downloadBlob(text, `${baseName}-r2b-report.md`, 'text/markdown');
      return;
    }
    if (format === 'zip') {
      const blob = await response.blob();
      downloadBlob(blob, `${baseName}-r2b-session.zip`, 'application/zip');
      return;
    }
    const data = await response.json() as AnalysisBundleResponse;
    downloadBlob(JSON.stringify(data, null, 2), `${baseName}-r2b-bundle.json`, 'application/json');
  }, [result?.binary, sessionId]);
