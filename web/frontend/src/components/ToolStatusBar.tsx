...
import type { ToolExecutionStatus, ToolsStatusResponse } from '../types';
...
const healthLabel = (kind: HealthKind): string => {
  if (kind === 'ok') return 'Available';
  if (kind === 'off') return 'Disabled for this run';
  return 'Unavailable';
};
...
const ToolStatusBar: FC<ToolStatusBarProps> = ({
  ...
  const visibleTools = useMemo(() => {
    ...
    const entries = Object.entries(status.tools).filter(([name]) => getToolCatalogEntry(name));
    if (!compact) return sortToolEntries(entries);
    return COMPACT_TOOL_ORDER
      .filter((name) => status.tools[name] && getToolCatalogEntry(name))
  ...
  const renderCard = (name: string, toolStatus: ToolExecutionStatus) => {
    ...
    const hint = health === 'down'
      ? (toolStatus.install_hint || toolStatus.details || 'Not available in this environment.')
      : catalog?.description;
